...
public class Robot {
	...
	//x and y coordinates of character's center
	private int centerX = 100;
	private int centerY = 377;
	
	private boolean jumped = false;//true when character in air
	...
	//rectangles around character parts for collisions
	...
	//the tiles in the immediate vicinity of robot 
	public static Rectangle yellowRed = new Rectangle(0, 0, 0, 0);
	//arrayList of bullets
	private ArrayList<Projectile> projectiles = new ArrayList<Projectile>();
	public int health = 30;

	//method update() called on each iteration of game loop
	public void update() {
		...
		if (centerX <= 200 && speedX > 0) {
			//moves character right if center of char <= 200
			centerX += speedX;
		} 
		if (speedX > 0 && centerX > 200) {
			//move the background at 1/5s of chars speed
		...
		if (centerX + speedX <= 60) {
			//if character is too far outside screen, fix the center
			centerX = 61;
		}
